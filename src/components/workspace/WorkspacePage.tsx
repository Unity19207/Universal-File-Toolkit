...
export function WorkspacePage() {
  ...
  const [module, setModule] = useState<Awaited<
    ReturnType<NonNullable<typeof tool>["load"]>
  > | null>(null);
  ...
  const isNoFileTool =
    ...
    (!tool?.accepts?.length && tool?.maxFileSize === 0);
  ...
  const applyFiles = (
    acceptedFiles: File[],
    source: "drop" | "paste" | "typed",
  ) => {
    if (!tool) return;
    const nextIssues = collectValidationSummary(
      validateFiles(acceptedFiles, tool),
    );
    ...
    if (nextIssues.length === 0) {
      ...
      if (source === "typed")
        pushToast("Pasted text attached as input.", "info");
    }
  };
  ...
  const usePastedContent = () => {
    ...
    const file = new File([pastedInput], `${tool.id}-pasted-input.txt`, {
      type: "text/plain",
    });
  ...
  const copyOutput = async () => {
    ...
    try {
      ...
      pushToast(
        "Clipboard write failed — try downloading instead.",
        "negative",
      );
    }
  };
  ...
  const runProcessing = async () => {
    ...
    if (updated?.status === "failed")
      pushToast(updated.error || "Processing failed.", "negative");
  };
  ...
  const activeStep =
    hasResult || isRunning ? 3 : inputCount > 0 || isNoFileTool ? 2 : 1;
  ...
  function formatAcceptedTypes(accepts: string[]): string {
    ...
    const names = accepts
      .map(
        (t) => typeMap[t.toLowerCase()] ?? t.split("/")[1]?.toUpperCase() ?? t,
      )
  ...
  return (
    <main className="workspace-page">
      ...
      {isNoFileTool ? (
        <div className="workspace-no-file">
          <div className="no-file-info-banner">
            ...
            <span>
              This tool generates output from your settings — no file upload
              needed.
            </span>
          </div>
              ...
              <div className="tool-options-form">
                {module && options !== null ? (
                  ...
                  <div className="space-y-4">
                    <Skeleton className="h-10 w-full r-sm" />
                    <Loader label="Loading engine..." />
                  </div>
                )}
              </div>
            </div>
          ...
          <button
            ...
            disabled={
              isRunning ||
              (inputCount === 0 && tool.requiresFile !== false) ||
              !module
            }
          >
            ...
            {showProgress &&
              (status === "succeeded" ? (
                <div className="job-success-banner">
                  ...
                  <span className="job-success-text">
          ...
          <div className="workspace-col-left">
            <div className="workspace-col-left-card">
              <div
                ...
                className={`drop-zone ${dropzone.isDragActive ? "drag-over" : ""}`}
              >
                ...
                <div className="drop-zone-meta">
                  {formatAcceptedTypes(tool.accepts)} · MAX 25 MB
                </div>
                ...
                {inputCount === 0 ? (
                  <div className="files-empty-state">
                    ...
                    <div className="files-empty-sub">
                      Your files stay in browser memory and are never uploaded
                    </div>
                  </div>
                ) : (
                    ...
                    {session?.inputs.map((file) => (
                      <div key={file.id} className="file-item">
                        ...
                        <button
                          ...
                          onClick={() => removeInput(tool.id, file.id)}
                        >
                          <svg
                            width="12"
                            height="12"
            ...
            {tool.supportsPaste && (
              <div className="source-card">
                <div className="source-card-header">
                  <div
                    className={`source-card-dot${!pastedInput ? " empty" : ""}`}
                  />
                ...
                <div className="source-card-footer">
                  <span className="source-card-status">
                    {pastedInput.trim()
                      ? `${pastedInput.trim().split("\n").length} lines ready`
                      : "No source applied"}
                  </span>
                  <div className="source-card-actions">
                    <button
                      className="source-clear-btn"
                      onClick={() => setPastedInput("")}
                      disabled={!pastedInput}
                    >
                      Clear
                    </button>
                    <button
                      className="source-apply-btn"
                      onClick={usePastedContent}
        ...
        <div className="output-panel-card">
            ...
            <div className="output-panel-actions">
              ...
              {session?.job.result?.outputs?.[0] && (
                <button
                  ...
                  onClick={() =>
                    downloadArtifact(session.job.result!.outputs[0])
                  }
                >
                  Download final
                </button>
          ...
          {hasResult && (
            <div className="output-panel-body">
              {/* Title + summary */}
              <div className="output-result-title">
                {session!.job.result!.preview.title}
              </div>
              {session!.job.result!.preview.summary && (
                <div className="output-result-summary">
                  {session!.job.result!.preview.summary}
                </div>
              )}
              ...
              {session!.job.result!.preview.kind === "image" && session!.job.result!.preview.objectUrl ? (
                ...
                session!.job.result!.outputs?.[0]?.type === "text/csv" ? (
                  <div className="output-preview-block overflow-hidden">
                    <CsvTablePreview
                      csvText={session!.job.result!.preview.textContent}
                      maxRows={15}
                    />
                  </div>
                  ...
                  <div className="output-preview-block">
                    <MarkdownTabs
                      md={session!.job.result!.preview.textContent}
                      meta={session!.job.result!.preview.metadata}
                      output={session!.job.result!.outputs?.[0]}
              ...
              {session?.job.result?.preview.metadata && (
                <div className="output-meta-strip">
                  ...
                  <div className="output-meta-pill">
                    ...
                    <span className="omp-value">
                      {session.job.result.outputs?.[0]?.name}
                    </span>
                  </div>
                  <div className="output-meta-pill">
                    <span className="omp-label">Size</span>
                    <span className="omp-value">
                      {formatBytes(session.job.result.outputs?.[0]?.size ?? 0)}
                    </span>
                  </div>
                </div>
              )}
              ...
              {session!.job.result!.outputs.length > 0 && (
                <div className="output-downloads-section">
                  <div className="output-downloads-label">
                    Available Downloads
                  </div>
                    ...
                    {session!.job.result!.outputs.map((out) => (
                      <div key={out.id} className="output-download-item">
                        <div className="min-w-0">
                          ...
                          <p className="output-download-size">
                            {formatBytes(out.size)}
                          </p>
                        </div>
                        <button
                          className="output-download-btn"
                          onClick={() => downloadArtifact(out)}
                        >
                          Download
                        </button>
                      </div>
      ...
      <section className="mt-12 pb-12">
        <p className="text-[11px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">
        ...
        <div>
              ...
              <div className="doc-section">
                ...
                <div className="doc-pills">
                  {tool.accepts.map((ext) => (
                    <span key={ext} className="doc-pill">
                      {ext}
                    </span>
                  ))}
                </div>
          ...
          <div className="doc-section">
            ...
            <div className="doc-pills">
              ...
              {tool.supportsBatch && (
                <span className="doc-pill">Batch processing</span>
              )}
              {tool.supportsPaste && (
                <span className="doc-pill">Paste support</span>
              )}
              {tool.maxFileSize && (
                <span className="doc-pill">
                  Max {Math.round(tool.maxFileSize / 1024 / 1024)} MB
                </span>
              )}
            </div>
          </div>
            ...
            {tool.requiresFile !== false ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <p className="doc-section-text">
                  1. Drop or click to select a file in the input panel.
                </p>
                <p className="doc-section-text">
                  2. Adjust settings in the Configuration panel on the right.
                </p>
                <p className="doc-section-text">
                  3. Click <strong>Process Files</strong> — all processing
                  happens locally in your browser.
                </p>
                <p className="doc-section-text">
                  4. Download or copy the result from the Preview &amp; output
                  panel.
                </p>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <p className="doc-section-text">
                  1. Configure the settings in the panel below.
                </p>
                <p className="doc-section-text">
                  2. Click <strong>Generate</strong> — everything runs in your
                  browser, nothing is sent to a server.
                </p>
                <p className="doc-section-text">
                  3. Download or copy the result from the Preview &amp; output
                  panel.
                </p>
              </div>
            )}
...
function processInline(text: string): string {
  return text
    .replace(
      /\[x\]/gi,
      '<input type="checkbox" disabled checked class="md-check">',
    )
...
function buildHTMLTable(lines: string[]): string {
  ...
  const thHTML = headers
    .map(
      (h, i) =>
        `<th style="text-align:${alignments[i]}">${processInline(h)}</th>`,
    )
    .join("");
    ...
    .map(
      (row) =>
        ...
        headers
          .map(
            (_, i) =>
              `<td style="text-align:${alignments[i]}">${processInline(row[i] ?? "")}</td>`,
          )
          .join("") +
        "</tr>",
    )
...
function buildList(
  ...
  while (i < lines.length) {
    ...
    if (ulMatch && type === "ul") {
      ...
      if (html.endsWith("</li>")) {
        html =
          html.slice(0, -5) + "<br>" + processInline(line.trim()) + "</li>";
      } else {
        break;
      }
    }
...
function renderMarkdownPreview(md: string): string {
  ...
  if (fmMatch) {
    ...
    const fmPairs = fmLines
      .map((line) => {
        const colon = line.indexOf(":");
        if (colon === -1)
          return `<div class="fm-line">${escapeHtml(line)}</div>`;
  ...
  html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    codeBlocks.push(
      `<div class="md-code-block">${lang ? `<span class="code-lang">${escapeHtml(lang)}</span>` : ""}<pre><code>${escapeHtml(code.trimEnd())}</code></pre></div>`,
    );
  ...
  while (i < lines.length) {
    ...
    if (hMatch) {
      ...
      const slug = hMatch[2]
        ...
        .replace(/\s+/g, "-");
      outputLines.push(
        `<h${level} class="md-h${level}" id="${slug}">${text}</h${level}>`,
      );
      i++;
      continue;
    }
    ...
    if (line.startsWith("&gt; ")) {
      ...
      outputLines.push(
        `<blockquote class="md-bq">${bqContent.trim()}</blockquote>`,
      );
      continue;
    }
...
function MarkdownTabs({ md, meta, output, downloadArtifact }: any) {
  ...
  const contentType =
    meta?.find((m: any) => m.label === "Content Type")?.value || "auto";
  ...
  return (
    <div className="mt-4 min-w-0 flex flex-col">
      ...
      <div className="md-tab-content relative">
        ...
        {activeTab === "preview" ? (
          <div
            className="md-preview-wrapper"
            dangerouslySetInnerHTML={{ __html: renderMarkdownPreview(md) }}
          />
        ) : (
